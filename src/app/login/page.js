...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (googleEnabled) {
        toast.dismiss(loadToast);
        // Trigger Better Auth real Google sign-in
        ...
        const targetEmail = prompt(
          "Enter your Google Email for simulation:",
          "mahmudjifan@gmail.com"
        );
        ...
        const simPassword = "GoogleSimulatedPassword123!";

        // Sign in or sign up dynamically
        ...
        if (!signInError) {
          toast.dismiss(t);
          toast.success(`Signed in as ${derivedName}!`);
          router.push("/");
          router.refresh();
          return;
        }
        ...
        const { error: signUpError } = await authClient.signUp.email({
          ...
          password: simPassword,
          name: derivedName,
          image: `https://api.dicebear.com/7.x/pixel-art/svg?seed=${namePart}`,
        });
      ...
      <div className="card w-full max-w-md bg-base-100 shadow-2xl border border-base-200">
        <div className="card-body gap-6">
          ...
          <button
            ...
            className={`btn btn-outline rounded-xl flex items-center gap-2 border-base-300 hover:bg-base-200 ${googleLoading ? "loading" : ""}`}
            disabled={loading || googleLoading}
          >
            {!googleLoading && (
